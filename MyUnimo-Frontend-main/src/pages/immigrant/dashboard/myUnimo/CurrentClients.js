...
import { Card, CardContent, Typography, Grid, Table, TableBody, TableCell, TableContainer,
  TableHead, TableRow, Paper, Select, MenuItem, Link, Button } from '@mui/material';
...
const CurrentClients = (props) => {
  ...
  const updateMyUnimoDetails = async (e, id, currentClientStatus, serviceId) => {
    e.preventDefault();
    const options = { id: id,  currentClientStatus: currentClientStatus, serviceId: serviceId};
    console.log("Options", options);

    const data = await postData('myunimo/updateCurrentState', options);
                ...
                <TableRow key={row.myUnimoNo} sx={{ '&:last-child td, &:last-child th': { border: 0.5 } }}>
                  ...
                  <TableCell style={{ border: '1px solid' }}>
                      <Select value={row.currentClientStatus}  style={{ width: '100%', height: '40px' }}
                              ...
                              {row.currentClientStatus === 'Decline' ?<MenuItem value='Decline' style={{ color: 'red' }}>Decline</MenuItem>:null}
                              <MenuItem value='Send for Approval' style={{ color: 'orange' }}>{row.currentClientStatus === 'Decline' ? "Resend":"Send"} for Approval</MenuItem>
                      </Select>
                  </TableCell>
                  ...
                  <TableCell style={{ border: '1px solid' }}>
                  <Button variant="contained" color="primary" sx={{ color: 'white' }} onClick={(e) => updateMyUnimoDetails(e, row.id, row.currentClientStatus, row.serviceId)}>
                    Update
                  </Button>
                  </TableCell>
                </TableRow>
