...
const AddMyUnimo = (props) => {
  ...
  const validationCheck = (e) =>{
    if(serviceId !== '' && serviceStepsId !== '' && clientEmail !== '') {
      e.preventDefault()
      sendInvitation()
    } else {
      setOpenAlert(true)
  ...
  const mailSend = async (id) =>{
    const options = {serviceName: serviceName, id: id, email: clientEmail}
  ...
  const sendInvitation = async () => {
      ...
      const options = {serviceName: serviceName, serviceId: serviceId, serviceStepsId: serviceStepsId,
                            ...
                            myUnimoNo: myUnimoNo, stepNo: stepNo}
    ...
    <Grid container spacing={4}>
          ...
          <CardContent>
            <Box sx={{ maxWidth: '100%' }} md={{ maxWidth: '100%'}}>
              <Grid container spacing={4}>
                  ...
                  <Grid item md={12} xs={12} className="paddingTopNone">
                    {/* ---Client Email --- */}
                    <Typography className="titleDesign" component="h6" variant="h6" >Business email address: *</Typography>
                  </Grid>
                  <Grid item md={12} xs={12} className="paddingTop">
                    <TextField size="small" fullWidth id="clientEmailAddress" placeholder="Business email address"
      ...
      <Grid item md={8} xs={12}>
        <Card style={{marginTop: '10px', height: '100%'}}>
          ...
          <CardContent>
            <ViewMyUnimo setTabValue={setTabValue}/>
          </CardContent>
        </Card>
      </Grid>
